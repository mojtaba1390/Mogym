    ...
    $('#form-tms').on('submit', function () {
        ...
        $('input[type="number"]').each(function () {
            if ($(':input[type="number"]').val() <= 0) {
                $(':input[type="number"]').closest('tr').find('.endDtate-td span').text('مقدار ساعت پایان نباید کوچکتر از ساعت شروع باشد')
                return false;
            } else {
                $(':input[type="number"]').closest('tr').find('.endDtate-td span').text(' ')
                return true;
            }
        });
